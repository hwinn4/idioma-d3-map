...
Adapters::DataParser.parse.each do |state_hash|
  ...
  StateLanguage.find_or_create_by(state_id: new_state.id, language_id: first_language.id, num_speakers: state_hash["speakers1"])

  second_language = Language.find_or_create_by(name: state_hash["language2"])
  StateLanguage.find_or_create_by(state_id: new_state.id, language_id: second_language.id, num_speakers: state_hash["speakers2"])
